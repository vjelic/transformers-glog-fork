...
class ModelArguments:
    ...
    resume: Optional[int] = field(default = 0)
...
def text_to_datasets(data_dir):
    ...
    try:
      ...
      f1=open(data_dir+"/ringbearer_ascii.txt","r").readlines()
      f1=' '.join(f1)
...
def test_tf(model_args, training_args):
    ...
    with strategy.scope():
        ...
        if model_args.resume:
            try:
                # load a previously saved partially fine-tuned model
        ...
        model.compile(optimizer=optimizer, loss="passthrough", metrics=[], steps_per_execution=1, jit_compile=True)
        if model_args.tf_fp16:
            if model_args.tf_fp16==1:
                model.optimizer.dynamic_scale.assign(256.)
